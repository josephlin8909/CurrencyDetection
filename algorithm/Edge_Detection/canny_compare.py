...
def gaussian_smoothing(image: np.ndarray, sigma: int, conv_type: int = 0):
    ...
    # The conv_type specified determines what convolution is used
    # where 1 is our convolution and 0 is the library convolve
    if conv_type:
        result = convolution(image, gaussian_window)
    else:
...
def gradient_calc(img: np.ndarray, conv_type: int = 0):
    ...
    if conv_type:
        x_gradient = convolution(img, sobel_x)
        y_gradient = convolution(img, sobel_y)
...
def edge_detection(img: np.ndarray, sigma: int, conv_type: int = 0):
    # Anvit Sinha 10/1/2022
    ...
    img_copy, theta = gradient_calc(img_copy, conv_type)
