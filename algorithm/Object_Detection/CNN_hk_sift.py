...
def load_hk_data(directory, getIndividual=None): 
  ...
  for image_class in os.listdir(directory) : 
    for image_name in os.listdir(os.path.join(directory, image_class)):
      ...
      img_with_keypoints = cv2.resize(img_with_keypoints, (300,300))
      image = cv2.resize(image, (300,300))
      image = np.dstack((image, img_with_keypoints))
      # normalize the image
...
cnn = Sequential() 
cnn.add(Conv2D(32, kernel_size=7, activation='relu', input_shape=(300,300,6), name='conv1'))
